...
const scatterChartPath = path.join(robustnessDir, 'scatter-net-worth-vs-regret.svg');
const cdfChartPath = path.join(robustnessDir, 'cdf-top-robust-strategies.svg');
...
const collectStrategyCandidates = () => {
  ...
  return Array.from(strategyMap.values())
    .map(({ sourceResult, key }, index) => ({
      strategyId: `S${String(index + 1).padStart(2, '0')}`,
      key,
...
const buildHeatmapCells = (metrics) => {
  ...
  const bestScore = Math.max(...cells.map((cell) => cell.compositeRobustScore));
  const bestFeasibility = Math.max(...cells.map((cell) => cell.weightedFeasibilityProbability));
...
const summarizePlateauRegion = (cells, metricsById) => {
  const plateauCells = cells.filter((cell) => cell.isPlateau);
  const cellsToUse = plateauCells.length > 0 ? plateauCells : [cells[0]];
...
const buildScatterSvg = ({ metrics, frontier, topStrategies }) => {
  const width = 960;
  const height = 560;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <text x="${padding.left}" y="24" font-size="20" font-family="Arial, sans-serif" font-weight="700" fill="#0f172a">Expected End Net Worth vs 10% CVaR Regret</text>
  <text x="${padding.left}" y="44" font-size="12" font-family="Arial, sans-serif" fill="#475569">Higher and further right is better net worth. Lower on the chart is better regret CVaR.</text>
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  <rect width="${width}" height="${height}" fill="#ffffff"/>
  <text x="${padding.left}" y="24" font-size="20" font-family="Arial, sans-serif" font-weight="700" fill="#0f172a">CDF of End Net Worth for Top Robust Strategies</text>
  <text x="${padding.left}" y="44" font-size="12" font-family="Arial, sans-serif" fill="#475569">Each line shows the weighted cumulative distribution of end net worth.</text>
  <line x1="${padding.left}" y1="${padding.top + plotHeight}" x2="${padding.left + plotWidth}" y2="${padding.top + plotHeight}" stroke="#94a3b8"/>
  <line x1="${padding.left}" y1="${padding.top}" x2="${padding.left}" y2="${padding.top + plotHeight}" stroke="#94a3b8"/>
  ${[0, 0.25, 0.5, 0.75, 1].map((tick) => {
...
const buildHeatmapSvg = ({ cells }) => {
  ...
  const minScore = Math.min(...cells.map((cell) => cell.compositeRobustScore));
  const maxScore = Math.max(...cells.map((cell) => cell.compositeRobustScore));
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <text x="${padding.left}" y="48" font-size="12" font-family="Arial, sans-serif" fill="#475569">Each cell shows the best strategy available for that starting deposit and mortgage pair. Bold outline marks the plateau region.</text>
  ${cells.map((cell) => {
    const x = getX(cell.deposit1);
    const y = getY(cell.mortgage1);
    const fill = colorScale(cell.compositeRobustScore, minScore, maxScore, 'e0f2fe', '0f766e');
    return `
      ...
      <text x="${x + 8}" y="${y + 33}" font-size="10" font-family="Arial, sans-serif" fill="#0f172a">${cell.compositeRobustScore.toFixed(1)}</text>
    `;
  }).join('')}
...
const reportJson = {
  ...
  topStrategies: topStrategies.map((metric, index) => ({
    rank: index + 1,
    strategyId: metric.strategyId,
  ...
  charts: {
    scatter: 'robustness-analysis/scatter-net-worth-vs-regret.svg',
    cdf: 'robustness-analysis/cdf-top-robust-strategies.svg',
...
await writeFile(scatterChartPath, scatterSvg);
await writeFile(cdfChartPath, cdfSvg);
